...
If you installed via the one-liner, the extension is already unpacked at **`~/pickmem-extension`** — just open `chrome://extensions` → enable **Developer mode** → **Load unpacked** → pick that folder. (Re-running the installer refreshes it in place; reload the extension afterward.) From a clone, build it instead:
